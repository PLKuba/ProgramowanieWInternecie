...
const ApproachClient = ({header_1, header_2, cards}: {
    header_1: string,
    header_2: string,
    cards: {
        order: string
    ...
    return (
        <section className="2-full py-20">
            <h1 className="heading">
                {header_1}<span className="text-purple">{header_2}</span>
            </h1>
            <div className="my-20 flex flex-col lg:flex-row items-center justify-center gap-4">
                {cards.map(({title, order, description, animationSpeed, containerClassName, colors, dotSize}) => (
                    <Card title={title} icon={<AceternityIcon order={order} />} description={description}>
                        <CanvasRevealEffect
  ...
  <span className="absolute inset-0 overflow-hidden rounded-full">
    <span
        className="absolute inset-0 rounded-full bg-[image:radial-gradient(75%_100%_at_50%_0%,rgba(56,189,248,0.6)_0%,rgba(56,189,248,0)_75%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100"/>
                ...
                <div
                    className="relative flex space-x-2 items-center z-10 rounded-full bg-zinc-950 px-5 py-2 ring-1 ring-white/10 text-2xl font-bold">
    <span>
      {order}
    </span>
                </div>
